...
from utils import (capitalize_names, list_cloned_voices, get_llm)
...
def build_message_list():
    ...
    transcript = df[df.presenter == selected_name_lower]['transcripts'].to_string(
        index=False)[400:]
...
# select presenter from the list of cloned voices
# grab video link by presenter
# show video
# check if transcript exists, if not, generate transcript
...
def generate_response():
    ...
    audio_stream = generate(text=ai_response.content,
                            stream=True, voice=v1, model="eleven_turbo_v2")
...
cloned_names_caps = [
    n for n in cloned_names_caps if n not in ('Ray Dalio', 'Dalai Lama')]
selected_name = st.sidebar.selectbox(
    "Select a TED Presenter", options=cloned_names_caps, on_change=initialize_session_state)
...
if st.session_state['generated']:
    for i in range(len(st.session_state['generated'])-1, -1, -1):
        ...
        message(st.session_state["generated"][i], key=str(
            i), logo=f'https://api.dicebear.com/7.x/initials/svg?seed={selected_name}')
